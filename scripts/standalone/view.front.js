...
async function translateText(text, target) {
  const res = await fetch("/api?action=translate", {
    method: "POST",
...
async function renderLyrics({ withProgress = false } = {}) {
  ...
  if (withProgress) {
    bodyEl.textContent = "";
    progress = mountProgress(loadingHost, { label: t.translating, etaMs: estimateEtaMs(6) });
  } else {
    bodyEl.textContent = t.translating;
  }
  try {
    bodyEl.textContent = await translateText(originalLyrics, lang);
  } catch {
    showOriginal();
...
async function loadLyrics() {
  ...
  const progress = mountProgress(loadingHost, { label: t.loading, etaMs: estimateEtaMs(3) });
  try {
