...
function ensureAddressColumns() {
  for (const sql of COLUMN_MIGRATIONS) {
    try {
      wranglerExec(`d1 execute ${ADDRESS_DB} --remote -y --command=${JSON.stringify(sql)}`, { cwd: root });
    } catch {
...
export function warmAddressRemote() {
  ...
  try {
    for (let i = 0; i < chunks.length; i++) {
      ...
      wranglerExec(`d1 execute ${ADDRESS_DB} --remote -y --file=${JSON.stringify(file)}`, { cwd: root, inherit: true });
      unlinkSync(file);
    }
  } finally {
    try {
