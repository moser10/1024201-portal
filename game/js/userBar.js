...
const TEXT = {
  zh: { hint: "点击用户名退出登录", logout: "退出登录" },
  en: { hint: "Tap username to sign out", logout: "Sign out" },
  ja: { hint: "ユーザー名をタップしてログアウト", logout: "ログアウト" },
  bo: { hint: "སྤྱོད་མཁན་མིང་ལ་བརྡབས་ནས་ཕྱིར་འཐེན།", logout: "ཕྱིར་འཐེན།" },
};
...
export function mountUserBar(container, options = {}) {
  ...
  if (!user) {
    ...
    link.href = loginUrl(returnPath);
    link.title = TEXT[lang]?.logout ? "Login" : "登录";
